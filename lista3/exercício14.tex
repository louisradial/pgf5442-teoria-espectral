...
\begin{proof}[Resolução de (b)]
  ...
  Voltando nossa atenção para o operador normal \(T\), temos \((T^*T)^{(2^n)} = \left[T^{(2^n)}\right]^* T^{(2^n)}\) para todo \(n \in \mathbb{N}.\) De fato, temos \((T^*T)^2 = T^*T T^*T = (T^*)^2 T^2 = (T^2)^*T^2\) e assumindo válido para algum \(k \in \mathbb{N}\) temos
  \begin{align*}
    (T^*T)^{(2^{k+1})} &= \left[(T^*T)^{(2^k)}\right]^2\\
                       &= \left\{\left[T^{(2^k)}\right]^*T^{(2^k)}\right\}^2\\
                       &= \left[T^{(2^k)}\right]^*T^{(2^k)}\left[T^{(2^k)}\right]^*T^{(2^k)}\\
                       &= \left[T^{(2^k)}\right]^*\left[T^{(2^k)}\right]^*T^{(2^k)}T^{(2^k)}\\
                       &= \left[T^{(2^{k+1})}\right]^*T^{(2^{k+1})}
  \end{align*}
  e concluímos válida para todo \(n \in \mathbb{N}\) pelo princípio da indução finita. Pela propriedade C*, temos então que
  \begin{equation*}
    \norm{T^{(2^k)}}^2 = \norm*{\left[T^{(2^k)}\right]^*T^{(2^k)}} = \norm{(T^*T)^{(2^n)}} = \norm{T^*T}^{(2^n)} = \norm{T}^{(2^{n+1})},
  \end{equation*}
  isto é, \(\norm{T^{(2^k)}} = \norm{T}^{(2^k)}.\)
  Consideremos agora a sequência \(\family{\norm{T^n}^{\frac1n}}{n\in \mathbb{N}}\) convergente para \(r(T)\). Assim, a subsequência \(\family{\norm*{T^{(2^k)}}^{\frac{1}{2^k}}}{k \in \mathbb{N}}\) converge para \(r(T)\) e temos
  \begin{equation*}

...
\begin{proof}[Resolução de (a)]
  ...
  \end{equation*}
  logo \(y \in D,\) isto é, \(\hilbert_2 = D.\) Ainda, temos para todo \(y \in \hilbert_2\) que
  \begin{equation*}
    \norm{T^*y}^2 = \abs{\inner{T^*y}{T^*y}} = \abs{\inner{y}{TT^*y}} \leq \norm{T} \norm{y} \norm{T^* y} \implies \norm{T^*y} \leq \norm{T}\norm{y},
  \end{equation*}
  portanto \(T^*\) é uma aplicação linear bem definida e limitada.
\end{proof}

...
\begin{proof}[Resolução]
  ...
  \begin{equation*}
    (\mathscr{F}f_a^+)(k) = \sqrt{\frac{a}{\pi}} \sinc(ka)\quad\text{e}\quad(\mathscr{F}f_a^-)(k) = \sqrt{\frac{a}{\pi}} \operatorname{cosc}(ka),
  \end{equation*}
  onde \(\sinc\) e \(\operatorname{cosc}\) são as funções contínuas definidas por \(\sinc(\xi) = \xi^{-1}\sin{\xi}\) e \(\operatorname{cosc}(\xi) = \xi^{-1}(1 - \cos\xi)\) para \(\xi \neq 0.\) Com isso, as funções \(g_a^{\pm1} = f_a^{+} \pm \mathscr{F}f_a^{+}\) e \(g_{a}^{\pm i} = f_a^{-} \mp i \mathscr{F} f_a^{-}\) são não nulas e satisfazem
  \begin{align*}
